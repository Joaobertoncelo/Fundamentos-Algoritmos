...
void inserirCarta(Carta carta[], char nom[], char tip[], char rari[], int el, int lvl,int numeroDeCartasDisponiveis){
	int i;
	for(i=0; i<50; i++){
		if (i == numeroDeCartasDisponiveis){
			...
			carta[i].nivel=lvl;
			carta[i].id=i+1;
			i=numeroDeCartasDisponiveis;// sair do for
		}
	}
}
void printarCartas(Carta carta[], int numeroDeCartasDisponiveis){
	int i;
	for(i=0; i<numeroDeCartasDisponiveis; i++){
		printf("%d-\n", carta[i].id);
		printf("Nome: %s\n", carta[i].nome);
		printf("tipo: %s\n", carta[i].tipo);
		printf("raridade: %s\n", carta[i].rar);
...
void printarNome(Carta carta[],int numeroDeCartasDisponiveis){
	int i;
	for(i=0; i<numeroDeCartasDisponiveis; i++){
...
int main(){
	int i, j=0, k=0, opcao, altNivel, novoNivel, numeroDeCartasDisponiveis=0, consult, deck;//a variavel aux serve para contar o numero de "switches" que sao executados
	...
	Carta novoDeck[8];
	Carta carta[50];
	Carta cartaArquivo[50];
	FILE *file;
	file = fopen("Cartas Disponiveis.txt", "r");  
	if(file == NULL){
		printf("Nao Foi Possivel Abrir o Arquivo.\n");
	}else{
		while(fgets(cartaArquivo[0].nome, 50, file) != NULL){
			fscanf(file, "%d %s %s %s %d %d", &cartaArquivo[j].id, &cartaArquivo[j].nome, &cartaArquivo[j].tipo, &cartaArquivo[j].rar, &cartaArquivo[j].elixir, &cartaArquivo[j].nivel);
			numeroDeCartasDisponiveis++;
		}
		if(cartaArquivo[0].nome!=NULL||cartaArquivo[0].tipo!=NULL||cartaArquivo[0].rar!=NULL||cartaArquivo[0].elixir>-1||cartaArquivo[0].nivel>-1){
			printarCartas(cartaArquivo, numeroDeCartasDisponiveis);
		}
	}
	fclose(file);
	inicializarVetor(carta);
	do{
		...
		switch (opcao){
	        case 1:
				...
				inserirCarta(carta,nome,tipo,rar,elixir,nivel, numeroDeCartasDisponiveis);
				printf("\n");
				numeroDeCartasDisponiveis++;
			break;
			case 2:
				...
				scanf("%d", &bomba);
				for(i=bomba-1; i < numeroDeCartasDisponiveis-1; i++){
					//explodir carta
					if(i!=numeroDeCartasDisponiveis){
			...
			break;
			case 3:
				printarCartas(carta,numeroDeCartasDisponiveis);
			break;
			case 4:
				printarNome(carta, numeroDeCartasDisponiveis);
				printf("\nDigite a ID da carta que deseja consultar: ");
				scanf("%d", &consult);
				for(i=0; i<numeroDeCartasDisponiveis; i++){
					if(carta[i].id==consult){
						printf("%d-\n", carta[i].id);
					    printf("Nome: %s\n", carta[i].nome);
					    printf("tipo: %s\n", carta[i].tipo);
					    printf("raridade: %s\n", carta[i].rar);
			...
			case 6:
				...
				for(i=0; i<8; i++){
					...
					scanf("%d", &deck);
					setbuf(stdin,NULL);
					for(k=0; k<numeroDeCartasDisponiveis; k++){
						if(carta[k].id==deck){
